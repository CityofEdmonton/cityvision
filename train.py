...
def download_data_from_gcs(bucket_name: str, gcs_path: str, local_dir: str) -> None:
    ...
    try:
        ...
        if downloaded_count == 0:
            print(f"No zipped files found or downloaded from gs://{bucket_name}/{gcs_path}. "
        ...
        print("You can use `gcloud auth application-default login` for local development or set the "
...
def update_traintxtfile(train_filename: str, individual_dataset_part_base: str) -> None:
    ...
    if train_filename:
        ...
        print(f"Updating train file: {train_filename} in {individual_dataset_part_base}")
        ...
        updated_lines = [os.path.join(individual_dataset_part_base, line.strip()) + '\n' for line in lines]
...
def manage_Image_Label_Folder(folder_path: str) -> None:
    ...
    if os.path.exists(os.path.join(folder_path, 'images')) and os.path.exists(os.path.join(folder_path, 'labels')):
        data_folder = os.path.join(folder_path, 'data')
        if not os.path.exists(data_folder):
            os.makedirs(data_folder)
        # Move images and labels to the data folder
        shutil.move(os.path.join(folder_path, 'images'), os.path.join(data_folder, 'images'))
        shutil.move(os.path.join(folder_path, 'labels'), os.path.join(data_folder, 'labels'))
...
def generate_master_yaml(dataset_root_folder, output_yaml_name="master_data.yaml"):
    ...
    subfolders = [d for d in os.listdir(dataset_root_folder) if os.path.isdir(os.path.join(dataset_root_folder, d))]

    if not subfolders:
        print(f"No subfolders found in '{dataset_root_folder}'. Please ensure your dataset parts are in subfolders.")
        return

    # Try to load class names from the first found data.yaml
    first_data_yaml_found = False
    print(f"Looking for 'data.yaml' files in subfolders of '{dataset_root_folder}' to extract class names...")
    # -----Getting the names section-----
    for folder in subfolders:
        print(f"Checking folder: {folder}")
        individual_data_yaml_path = os.path.join(dataset_root_folder, folder, "data.yaml")
        if os.path.exists(individual_data_yaml_path):
            try:
                ...
                if not master_names:
                    print(f"Warning: 'names' section not found in '{individual_data_yaml_path}'. Please ensure your data.yaml files define class names.")
                else:
                    print(f"Class names loaded from: {individual_data_yaml_path}")
                first_data_yaml_found = True
                break # Found names, break out
            except yaml.YAMLError as e:
                print(f"Error parsing YAML file '{individual_data_yaml_path}': {e}. Skipping for names.")
            except FileNotFoundError:
                # This should not happen if os.path.exists was true, but as a safeguard
                print(f"Error: data.yaml file '{individual_data_yaml_path}' disappeared unexpectedly. Skipping.")
    if not first_data_yaml_found:
        print("Error: No 'data.yaml' found in any subfolder to extract class names. Cannot generate master YAML.")
        return

    # Collect train and val paths
    for folder in subfolders:
        full_folder_path = os.path.join(dataset_root_folder, folder)
        manage_Image_Label_Folder(full_folder_path) # Ensure the folder is managed correctly
        print(f"Processing folder: {folder}")
        individual_data_yaml_path = os.path.join(dataset_root_folder, folder, "data.yaml")
        if not os.path.exists(individual_data_yaml_path):
            print(f"Warning: '{individual_data_yaml_path}' not found. Skipping folder '{folder}'.")
            continue
        try:
            with open(individual_data_yaml_path, 'r') as f:
            ...
            individual_yaml_base_path_in_yaml = data.get('path', '.') # This will be '.'
            ...
            update_traintxtfile(full_train_path, individual_dataset_part_base) # Update the train.txt file
            if train_filename:
                ...
                combined_train_path = os.path.join(individual_dataset_part_base, train_filename)
                master_train_paths.append(combined_train_path)
            else:
                print(f"Warning: 'Train' path not found in '{individual_data_yaml_path}'. Skipping training data from this part.")
            ...
            if val_filename:
                ...
                combined_val_path = os.path.join(individual_dataset_part_base, val_filename)
                master_val_paths.append(combined_val_path)
            # TODO: Handle case where no validation path is found
            else:
                print(f"Info: No 'Val' or 'val' path found in '{individual_data_yaml_path}'. "
                      "This part will not contribute validation data to the master YAML.")

        except yaml.YAMLError as e:
            print(f"Error parsing YAML file '{individual_data_yaml_path}': {e}. Skipping folder '{folder}'.")
        except Exception as e:
            print(f"An unexpected error occurred processing '{individual_data_yaml_path}': {e}. Skipping folder '{folder}'.")
    ...
    if master_val_paths:
        ...
        master_val_paths.append(master_train_paths[-1])  # Use the last train path as a fallback
        master_train_paths.pop()  # Remove the last train path since we are using it for validation
        master_data_content['val'] = master_val_paths # Add the fallback validation path
...
if __name__ == "__main__":
    ...
    if not os.path.exists(master_yaml_path):
        print(f"Error: {master_yaml_path} not found. Please ensure your GCSing and unzipping steps were successful.")
        exit(1)
